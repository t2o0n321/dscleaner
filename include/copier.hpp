...
class Copier {
public:
    // Local recursive copy of src -> dst, skipping junk. If dst is an existing
    // directory, src is copied *into* it. Returns 0 on success.
    int cp(const fs::path& src, const fs::path& dst, bool verbose);

    // Remote transfer wrapper. Prefers `rsync -e ssh` with --exclude (so junk
    // never crosses the wire); falls back to `scp -r` with a warning if rsync
    // is unavailable. `args` are the user's source(s) + destination, passed
    // straight through. Returns the underlying tool's exit code.
    int scp(const std::vector<std::string>& args, bool verbose);

    // Create an archive (tar/tar.gz/tar.bz2/tar.xz/zip, chosen by the output
    // extension) from the given sources, excluding junk. Returns the archiver's
    // exit code.
